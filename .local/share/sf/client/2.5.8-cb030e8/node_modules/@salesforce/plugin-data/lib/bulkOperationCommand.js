...
class BulkOperationCommand extends BulkBaseCommand_1.BulkBaseCommand {
    ...
    static printBulkErrors(failedResults) {
        const columns = {
            id: { header: 'Id' },
            sfId: { header: 'Sf_Id' },
            error: { header: 'Error' },
        ...
        core_2.ux.log();
        core_2.ux.table(failedResults.map((f) => ({ id: 'Id' in f ? f.Id : '', sfId: f.sf__Id, error: f.sf__Error })), columns, options);
    }
    async runBulkOperation(sobject, csvFileName, connection, wait, verbose, operation, options) {
        ...
        try {
            ...
            try {
                ...
                if (!(0, bulkUtils_1.isBulkV2RequestDone)(jobInfo)) {
                    return result;
                }
                if (this.jsonEnabled()) {
